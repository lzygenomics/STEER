...
class ClusterPredictionNetwork(nn.Module):
    def __init__(self, input_dim, num_cluster):
        ...
        self.fc1 = nn.Linear(input_dim, 64)
        self.fc2 = nn.Linear(64, num_cluster)
...
class ExpertModel(nn.Module):
    def __init__(self, ein_features, eout_features, ehidden_size=256):
        ...
        self.network = nn.Sequential(
            ...
            nn.Linear(ehidden_size, 512),
            nn.LayerNorm(512),  # Adding BatchNorm after the second hidden layer activation
            nn.LeakyReLU(),
            
            nn.Linear(512, 1024),
            nn.LayerNorm(1024),  # Adding BatchNorm after the third hidden layer activation
            nn.LeakyReLU(),
            
            nn.Linear(1024, eout_features),
...
def model_training_share_neighbor_adata(device, device2, pyg_data, MODEL_MODE, adata, NUM_LOSS_NEIGH, max_n_cluster, corr_mode = 'u', cos_batch = 100, path = '/HOME/scz3472/run/GATVelo/', mask_train = False, prior_time = None, order = None):
    ...
    patience_counter = 0
    # Training loop
    num_epochs = 50000 # 10000 before
    pretrain_epochs = 3000 # 1500 before
    cluster_epochs = 3000 # 1500 before
    time_epochs = 1000 # 200 before
    warm_up_epochs = 200 #200 before
    log_interval = 50
    ...
    for epoch in range(MAX_EPOCH):
        ...
        if epoch < pretrain_epochs:
            ...
            nn.utils.clip_grad_norm_(model.parameters(), max_norm=10)
            optimizer.step()
            # Apply scheduler and early stopping logic in Phase 2 when MODEL_MODE is pretrain
            if MODEL_MODE == 'pretrain':
                scheduler.step()
                ...
                if patience_counter > early_stopping_patience:
                    ...
                    print(f"Best model saved from epoch {best_epoch} with loss {best_loss}")
