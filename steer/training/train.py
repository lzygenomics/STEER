...
class GATModel(nn.Module):
    ...
    def forward(self, x, edge_index, use_initial_regulation=True, indices: Optional[Tensor] = None):
        ...
        # Compute cell parameters (cp)
        # If `indices` is provided, only compute cp for the selected subset of cells (cp batching)
        if indices is not None:
            z_sub = z[indices]
            t_sub = t[indices]
            c_sub = c_softmax[indices]
            cp = self.compute_cell_params(c_sub, z_sub, t_sub, self.num_genes)
        else:
...
def velocity_loss_share_single_top3_polish(
    pyg_data,
    up_type_features,
    cp,
    warm_up,
    ...
    """Velocity cosine loss with optional batching.

    Modes:
      1) Full mode (batch_indices is None):
         - cp is [N, 3*G]
         - optional internal batching across source cells via batch_size

      2) Stochastic cell batching mode (batch_indices is not None):
         - cp is [B, 3*G] computed only for batch_indices (cp batching)
         - only edges whose *source* is in batch_indices contribute to the loss

    Notes:
      - Top-k aggregation (k=3 by default) is performed over neighbor edges *per source cell*.
      - This function keeps the original per-gene cosine definition.
    """

    # ========= Mode 2: stochastic cell batching (recommended for large graphs) =========
    if batch_indices is not None:
        # Shapes
        num_cells, num_genes = pyg_data.unsplice.shape
        B = int(batch_indices.numel())

        # Edge index (already masked by time in get_mask_t_share)
        src_all = pyg_data.time_edge_index[0]
        tgt_all = pyg_data.time_edge_index[1]

        # Keep only edges whose source node is in batch_indices
        edge_mask = torch.isin(src_all, batch_indices)
        if not edge_mask.any():
            return torch.tensor(0.0, device=cp.device, requires_grad=True)

        src_batch = src_all[edge_mask]
        tgt_batch = tgt_all[edge_mask]

        # Map global cell id -> local id in cp (0..B-1)
        lookup = torch.full((num_cells,), -1, dtype=torch.long, device=cp.device)
        lookup[batch_indices] = torch.arange(B, device=cp.device)
        local_src_idx = lookup[src_batch]

        # Reshape cp: [B, G, 3] -> alpha/beta/gamma per (cell,gene)
        cp_reshaped = cp.view(B, num_genes, 3)
        alpha_all = cp_reshaped[..., 0]
        beta_all = cp_reshaped[..., 1]
        gamma_all = cp_reshaped[..., 2]

        alpha = alpha_all[local_src_idx]
        beta = beta_all[local_src_idx]
        gamma = gamma_all[local_src_idx]

        # Warm-up: override alpha by up-regulation sign if up_type_features is per-cell
        if warm_up and up_type_features is not None:
            if up_type_features.dim() == 2 and up_type_features.shape[0] == num_cells:
                curr_up_reg = up_type_features[src_batch]
                alpha = torch.where(
                    curr_up_reg > 0,
        ...
        # Expression data for src/tgt
        unsplice_src = pyg_data.unsplice[src_batch]
        splice_src = pyg_data.splice[src_batch]
        unsplice_tgt = pyg_data.unsplice[tgt_batch]
        splice_tgt = pyg_data.splice[tgt_batch]

        # Predicted velocity (per-edge, per-gene)
        pred_vu_edge = alpha - beta * unsplice_src
        pred_vs_edge = beta * unsplice_src - gamma * splice_src

        # Observed diff along edge
        unsplice_diff = unsplice_tgt - unsplice_src
        splice_diff = splice_tgt - splice_src
        ...
        # Aggregate per source cell (top-k over neighbor edges)
        _, inverse_indices = src_batch.unique(return_inverse=True)
        final_loss = masked_max_aggregation(cosine_sim, inverse_indices, k=k)
        return final_loss

    # ========= Mode 1: full graph loss (original implementation) =========
    # 1. Precompute global variables (small enough: [N_cells, N_genes])
    ...
    # Predicted velocity for all cells
    ...
    # Internal batching over unique source cells
    unique_src_cells = src_all.unique()
...
def post_loss_share(device2, z, pyg_data, cp, up_type_features, t, c_softmax, n_neighbors, warm_up, time_only=False, corr_mode='u', cos_batch=500, fine_cluster_vec=None, velo_batch_size=None, batch_indices: Optional[Tensor] = None):
    ...
    if time_only:
        ...
        if device2 != t.device:
            ...
            velo_loss = velocity_loss_share_single_top3_polish(pyg_data, up_type_features, cp, warm_up, batch_size=velo_batch_size, batch_indices=batch_indices)
...
def model_training_share_neighbor_adata(device, device2, pyg_data, MODEL_MODE, adata, 
    ...
    for epoch in range(MAX_EPOCH):
        ...
        # [Cell batching] Randomly sample a subset of cells each epoch for cp + velocity loss
        batch_indices = None
        if use_experts and (velo_batch_size is not None):
            num_cells = pyg_data.x.size(0)
            bsz = int(min(int(velo_batch_size), int(num_cells)))
            # Sample without replacement
            batch_indices = torch.randperm(num_cells, device=device)[:bsz]
        ...
        if epoch < pretrain_epochs:
            ...
            if mask_train:
                ...
                recon, z, t, c_softmax, cp, up_type_features = model(x_unmasked, pyg_data.edge_index, use_initial_regulation=True, indices=batch_indices)
                ...
                recon, z, t, c_softmax, cp, up_type_features = model(pyg_data.x, pyg_data.edge_index, use_initial_regulation=True, indices=batch_indices)
                GATE_loss, _ = pretrain_loss_function(data = pyg_data.x, recon = recon, z = z, adj = pyg_data.adj, c = c_softmax, edge_index = pyg_data.edge_index, n_neighbors = NUM_LOSS_NEIGH)
            loss = GATE_loss
            loss.backward()
            ...
            if mask_train:
                mask = generate_mask(pyg_data.x, 0.1)
                x_unmasked = pyg_data.x * (1 - mask)
                recon, z, t, c_softmax, cp, up_type_features = model(x_unmasked, pyg_data.edge_index, use_initial_regulation=False, indices=batch_indices)
                GATE_loss, cluster_loss = pretrain_loss_function(data = pyg_data.x, recon = recon, z = z, adj = pyg_data.adj, c = c_softmax, edge_index = pyg_data.edge_index, n_neighbors = NUM_LOSS_NEIGH)
            else:
                recon, z, t, c_softmax, cp, up_type_features = model(pyg_data.x, pyg_data.edge_index, use_initial_regulation=False, indices=batch_indices)
                GATE_loss, cluster_loss = pretrain_loss_function(data = pyg_data.x, recon = recon, z = z, adj = pyg_data.adj, c = c_softmax, edge_index = pyg_data.edge_index, n_neighbors = NUM_LOSS_NEIGH)
            # 【修改】传入 fine_clus_vec
            velo_loss, time_cor, temp_smooth_loss = post_loss_share(
                ...
                corr_mode=corr_mode,
                fine_cluster_vec=fine_clus_vec, # <--- Use Fine Cluster
                velo_batch_size=None if batch_indices is not None else velo_batch_size # <--- 传入
                , batch_indices=batch_indices
            )
    ...
    with torch.no_grad():
        ...
        num_genes = adata.n_vars
        recon, z, t, c_softmax, cp, up_type_features = model(pyg_data.x, pyg_data.edge_index, use_initial_regulation=False, indices=None)
...
# Function to train and evaluate for each gene
def model_training_share_neighbor_adata_ablation_MinCUT(device, device2, pyg_data, MODEL_MODE, adata, 
                                        NUM_LOSS_NEIGH, max_n_cluster, corr_mode = 'u', 
                                        cos_batch = 100, path = '/HOME/scz3472/run/GATVelo/', mask_train = False, prior_time = None, order = None,
    ...
    for epoch in range(MAX_EPOCH):
        ...
        if epoch < pretrain_epochs:
            ...
            loss = GATE_loss + time_cor + temp_smooth_loss # + cluster_loss
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), max_norm=10)
            optimizer.step()
        elif epoch <= pretrain_epochs + cluster_epochs + time_epochs + warm_up_epochs:
            ...
            loss = GATE_loss + velo_loss + time_cor + temp_smooth_loss # + cluster_loss 
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), max_norm=10)
            optimizer.step()
        else:
